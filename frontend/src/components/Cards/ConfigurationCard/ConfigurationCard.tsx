  ...
  return (
    <div className="configurationCard">
      <div className="configurationCard-part">
        <div className="configurationCard-type">
          {configuration?.type} {configuration.Configuration?.CarBrand?.name}{' '}
          {configuration.Configuration?.CarModel?.name} {configuration.Configuration?.CarGeneration?.name}
        </div>
        <div className="configurationCard-client">
          {configuration.Client?.first_name} {configuration.Client?.last_name}
        </div>
      ...
      <div className="configurationCard-part configurationCard-part-specs">
        {[...(specs || [])].map(([key, value]: any, index: number) => {
          ...
          return isKeyBanned ? (
            ...
            <div key={index} className="configurationCard-specs">
      ...
      <div className="configurationCard-part">
        ...
        {user?.role !== 'Client' ? (
          <Button
            onClick={() => {
              navigate(`/carselector/car/add/${configuration.Configuration?.id}`);
            }}
            name={'Add Car'}
            type={false}
          />
