...
const login = async (req: Request, res: Response) => {
  ...
  if (
    (!clientWithEmail || !comparePasswords(password, clientWithEmail.password!)) &&
    (!employeeWithEmail || !comparePasswords(password, employeeWithEmail.password!))
  ) {
...
const logout = async (req: Request, res: Response) => {
  ...
  if (typeof accessTokenPayload === 'object') {
    await invalidateSession(accessTokenPayload?.sessionId);
