...
export const requireAuthentication = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (accessTokenPayload && !isAccessTokenExpired) {
    if (
      !(await isSessionValid(typeof accessTokenPayload === 'object' ? accessTokenPayload.sessionId : req.session!.id))
    ) {
      return res.sendStatus(StatusCodes.UNAUTHORIZED);
    }
  ...
  const { payload: refreshTokenPayload } =
    isAccessTokenExpired && refreshToken ? verifyJWT(refreshToken) : { payload: null };

  if (!refreshTokenPayload) {
    return res.sendStatus(StatusCodes.UNAUTHORIZED);
  }

  const sessionId = typeof refreshTokenPayload === 'object' ? refreshTokenPayload.sessionId : req.session!.id;
